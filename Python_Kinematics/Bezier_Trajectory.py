...
def generate_trajectory(start_pos, end_pos, cp1, cp2, start_roll, end_roll,
    ...
    for t in t_values:
        ...
        p = B0*(1-s)**3 + B1*3*s*(1-s)**2 + B2*3*(s**2)*(1-s) + B3*s**3
        p = np.array(p).flatten()

        cur_x, cur_y, cur_z = p[0], p[1], p[2]
        current_pos = [float(cur_x), float(cur_y), float(cur_z)]

        v = 3*(1-s)**2*(B1 - B0) + 6*(1-s)*s*(B2 - B1) + 3*s**2*(B3 - B2)  # dp/dt
...
if __name__ == "__main__":
    ...
    # Define waypoints
    pt_home  = {'pos': [-35, 40, 55.0], 'cp': [15.54,   15.02, 25.54],  'roll': 52,  'pitch': -12, 'yaw':  25}
    pt_pick  = {'pos': [51, 42, 50],    'cp': [-20.52,  16.56, 40.28], 'roll': -10, 'pitch':  14, 'yaw': -24} 
    pt_lift  = {'pos': [60, -5, 40],    'cp': [41.55,  20.45, 37.64],  'roll': 41,  'pitch': -37, 'yaw': -45}
    pt_place = {'pos': [50, -50, 10],   'cp': [42.37, 26.61, 41.92], 'roll': -80, 'pitch':  18, 'yaw': -64}  

    traj1 = generate_trajectory(pt_home['pos'], pt_pick['pos'],
                                pt_home['cp'], pt_pick['cp'],
