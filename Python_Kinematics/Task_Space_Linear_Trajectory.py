...
def generate_trajectory(start_pos, end_pos, start_roll, end_roll,
    ...
    for t in t_values:
        cur_x     = start_pos[0] + t * (end_pos[0] - start_pos[0])
        cur_y     = start_pos[1] + t * (end_pos[1] - start_pos[1])
        cur_z     = start_pos[2] + t * (end_pos[2] - start_pos[2])
        ...
        if cur_angles[0] is not None:
            trajectory_data.append((cur_angles, current_pos, cur_orientation, (cur_roll, cur_pitch, cur_yaw)))
        else:
...
if __name__ == "__main__":
    ...
    # Unpack pos for each point in trajectory
    ...
    for i, (angles, pos, orientation, rpy) in enumerate(full_trajectory):
        j1, j2, j3, j4, j5, j6 = np.degrees(angles)
        px, py, pz = pos
        r, p, y = rpy
