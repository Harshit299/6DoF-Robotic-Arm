...
def generate_trajectory(start_pos, end_pos, start_roll, end_roll,
    ...
    start_orientation = return_cur_orientation(start_roll, start_pitch, start_yaw)
    end_orientation = return_cur_orientation(end_roll, end_pitch, end_yaw)

    start_angles = inverse_kinematics(start_pos, start_orientation)
    end_angles = inverse_kinematics(end_pos, end_orientation)
    ...
    for t in t_values:
        ...
        for i in range(6):
            ...
            # interpolating joint velocities
            cur_vel = v * (end_angles[i] - start_angles[i])
            cur_vels.append(cur_vel)

            # interpolating joint accelerations
            cur_acc = a * (end_angles[i] - start_angles[i])
        ...
        _, _, _, _, _, _, _, _, p9, _ = extract_coords(t1, t2, t3, t4, t5, t6)

        cur_pos = [p9[0], p9[1], p9[2]]

        if cur_angles[0] is not None:
            trajectory_data.append((cur_angles, cur_pos, cur_vels, cur_accs, cur_orientation, (cur_roll, cur_pitch, cur_yaw)))
        else:
...
if __name__ == "__main__":
    ...
    # Generating Joint angle profiles
    theta1_list = []
    theta2_list = []
    theta3_list = []
    theta4_list = []
    ...
    for angles, _, vel, acc, _, _ in full_trajectory:
        theta1_list.append(angles[0])
        theta2_list.append(angles[1])
        theta3_list.append(angles[2])
        theta4_list.append(angles[3])
        theta5_list.append(angles[4])
        theta6_list.append(angles[5])

        v1_list.append(vel[0])
        v2_list.append(vel[1])
        v3_list.append(vel[2])
        v4_list.append(vel[3])
        v5_list.append(vel[4])
        v6_list.append(vel[5])

        a1_list.append(acc[0])
        a2_list.append(acc[1])
        a3_list.append(acc[2])
        a4_list.append(acc[3])
        a5_list.append(acc[4])
        a6_list.append(acc[5])
    ...
    plt.plot(time, theta3_list, label="Theta3")
    plt.plot(time, theta4_list, label="Theta4")
    plt.plot(time, theta5_list, label="Theta3")
    plt.plot(time, theta6_list, label="Theta4")
    ...
    plt.plot(time, v1_list, label="Theta1")
    plt.plot(time, v2_list, label="Theta2")
    plt.plot(time, v3_list, label="Theta3")
    plt.plot(time, v4_list, label="Theta4")
    plt.plot(time, v5_list, label="Theta3")
    plt.plot(time, v6_list, label="Theta4")
    ...
    plt.plot(time, a1_list, label="Theta1")
    plt.plot(time, a2_list, label="Theta2")
    plt.plot(time, a3_list, label="Theta3")
    plt.plot(time, a4_list, label="Theta4")
    plt.plot(time, a5_list, label="Theta3")
    plt.plot(time, a6_list, label="Theta4")

    # Jerk profiles
    plt.xlabel("Time")
    plt.ylabel("Joint acceleration")
    plt.title("Joint Jerk profiles")
    plt.legend()
    plt.grid(True)
    plt.show()
    ...
    for i, (angles, pos, _, _, _, rpy) in enumerate(full_trajectory):
        j1, j2, j3, j4, j5, j6 = np.degrees(angles)
        px, py, pz = np.asarray(pos).reshape(3,)
        r, p, y = rpy
